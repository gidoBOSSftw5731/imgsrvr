...
DEPRECATED: coinhive shut down, so this is a sloppy removal of all the code since it looks like its breaking stuff
...
func verifyCCaptcha(resp http.ResponseWriter, req *http.Request, config config) (string, error) {
	...
	defer w.Body.Close() // there was a close big comment before
