...
func verifyCCaptcha(resp http.ResponseWriter, req *http.Request, config config) (string, error) {
	...
	body := strings.NewReader(fmt.Sprintf(`token=%v&hashes=1024&secret=%v`, config.coinhiveCaptchaPub, config.coinhiveCaptchaPriv))
	r, err := http.NewRequest("POST", "https://api.coinhive.com/token/verify", body)
	if err != nil {
...
func upload(resp http.ResponseWriter, req *http.Request, config config) /*(string, error)*/ {
	...
	verifyCCaptcha(resp, req, config)
